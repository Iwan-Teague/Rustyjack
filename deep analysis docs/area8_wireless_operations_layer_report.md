...
This report follows the evidence/trust rule stated in Architecture Doc 2: constraints come only from **root docs** and **`logs/done/`**, while code is treated as implementation evidence. fileciteturn2file6L8-L13  
This section is explicitly scoped to the “Wireless operations layer” responsibilities and analysis prompts in that doc. fileciteturn2file7L20-L55
...
The repo’s “big” safety invariant is interface isolation: **one uplink admin‑UP at steady state; others forced DOWN; non-selected wireless should be rfkill soft-blocked** (`logs/done/interface_isolation_overview.md`, root `TESTING.md`). fileciteturn2file4L29-L36
...
> **Hardware baseline constraint (trusted docs):** the **Pi Zero 2 W built‑in Wi‑Fi is not suitable for monitor/injection**; a USB adapter is required for those features (root docs + wireless notes). Example: `AGENTS.md` “wireless note” + `logs/done/wireless_*` docs. fileciteturn2file7L41-L45
...
- **Driver/phy capability query:** `rustyjack_netlink::WirelessManager::get_phy_capabilities()`  
  used in `rustyjack-wireless/src/nl80211_queries.rs` to compute:
  - `supports_monitor_mode`
  - `supports_tx_in_monitor` (aka injection-in-monitor)
  - `supports_ap_mode`, `supports_5ghz`, etc.
- **Front-door capability report:** `rustyjack-core/src/wireless_native.rs::check_capabilities()`  
...
**rfkill semantics note:** rfkill distinguishes software “soft block” vs hardware “hard block” (cannot be undone in software). citeturn1search6  
...
### What’s in tree
- `rustyjack-wireless/src/radiotap.rs`: minimal radiotap parser producing `RadiotapInfo`
- `rustyjack-wireless/src/frames.rs`: minimal 802.11 parser producing `Ieee80211Frame`
...
1) **Radiotap present bitmap handling is incomplete**  
   ...
   Radiotap itself is designed to be extensible; ignoring the extended bitmap is a correctness footgun. citeturn1search0

2) **802.11 header length is not universally 24 bytes**  
   Data/QoS frames and “4-address” cases change header layout. A parser that assumes fixed offsets can still be memory-safe but semantically wrong, which becomes a **policy bug** (filters, capture selection, gating decisions). IEEE 802.11 frame control encodes variants that require conditional parsing. citeturn1search1
...
- Disruptive operations must be **explicitly confirmed** (root docs, referenced by Architecture Doc 2). fileciteturn2file7L41-L45
- “Stealth/passive” posture must be truthfully represented: passive mode reduces emissions but is not magic RF invisibility. (trusted note) fileciteturn2file1L9-L13

### Guardrails that exist in code (implementation evidence)
- **Offensive gating via “review approval”:** `rustyjack-core/src/operations.rs::offensive_review_approved()`  
...
## 5) Findings (17)  
Format required: **Problem → Why → Where → Fix → Fixed version looks like**. fileciteturn2file6L23-L24
...
- **Why:** Radiotap explicitly supports extended present bitmaps and field alignment; ignoring these can mis-locate the 802.11 payload boundary and break parsing. citeturn1search0  
...
- **Why:** This yields semantic misclassification (e.g., addresses/QoS), which becomes policy risk if filters/UX depend on correct identification. citeturn1search1  
...
**Tooling:** `cargo-fuzz` (libFuzzer integration) is the standard Rust path. citeturn1search4turn1search7  
...
- Target: `parse_nested_attrs` + scan result decode path in `rustyjack-netlink/src/wireless.rs`
- Properties:
  - never infinite loop
  - never panic
...
- Hard-block Wi‑Fi via hardware switch (if available) and confirm UX reports inability to unblock (soft vs hard) citeturn1search6
