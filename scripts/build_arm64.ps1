...
$DockerRun = Join-Path $RepoRoot "docker\arm64\run.ps1"
$BuildMode = "debug"
$BuildProfileFlag = ""
$DefaultBuild = $false
$ContainerArgs = @()
...
if ($args.Count -gt 0) {
    ...
    if (Test-Interactive) {
        ...
        Prompt-UsbExportEarly -Arch "arm64" -Variant $UsbVariant
    } else {
...
if ($DefaultBuild) {
    ...
    $DestDir = Join-Path $RepoRoot "prebuilt\arm64\$PrebuiltVariant"
    ...
    Invoke-UsbExport -Arch "arm64" -Variant $PrebuiltVariant -SourceDir $DestDir
}
