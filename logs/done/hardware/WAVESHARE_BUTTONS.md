...
- Global, unified mapping for buttons is implemented in `crates/rustyjack-ui/src/app.rs`:
...
Files changed
- `crates/rustyjack-ui/src/app.rs` — added `ButtonAction` mapping, `MenuState::home()`, `map_button()`, `confirm_reboot()`, turned `show_message()` into a manual-dismiss dialog, and updated the main loop & a few controls to use the unified mapping.
