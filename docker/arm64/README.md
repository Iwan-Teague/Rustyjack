ARM64 build container (Pi Zero 2 W on 64-bit Pi OS or other ARM64 Pis). This sets up Rust with the aarch64 target and the minimal native deps needed for `rustyjack-ui`.
...
Inside the container, build for 64-bit Pi:
```bash
cargo build --target aarch64-unknown-linux-gnu -p rustyjack-ui
```
