...
impl AttackPipeline {
    ...
    fn build_password_pipeline(&self) -> Vec<AttackTechnique> {
        ...
        if self.config.stealth_mode {
            ...
            techniques.push(AttackTechnique::SetTxPower {
    ...
    pub fn describe(&self) -> Vec<String> {
        ...
        for (i, tech) in techniques.iter().enumerate() {
            let desc = match tech {
                ...
                AttackTechnique::EvilTwin { ssid, .. } => format!("Evil Twin AP - fake {}", ssid),
                ...
                AttackTechnique::QuickCrack { .. } => "Quick crack - common passwords".to_string(),
                AttackTechnique::DictionaryCrack { .. } => "Dictionary attack".to_string(),
                AttackTechnique::RandomizeMac => "Randomize MAC - stealth".to_string(),
                AttackTechnique::RestoreMac => "Restore original MAC".to_string(),
                AttackTechnique::SetTxPower { level } => format!("Set TX power: {}", level),
            };
...
mod tests {
    ...
    fn test_password_pipeline_has_required_steps() {
        let config =
            presets::quick_password("wlan0", Some("TestNetwork"), Some("AA:BB:CC:DD:EE:FF"));
        ...
        assert!(techniques
            .iter()
            .any(|t| matches!(t, AttackTechnique::RandomizeMac)));

        // Should include PMKID
        assert!(techniques
            .iter()
            .any(|t| matches!(t, AttackTechnique::PmkidCapture { .. })));

        // Should include deauth
        assert!(techniques
            .iter()
            .any(|t| matches!(t, AttackTechnique::Deauth { .. })));

        // Should include crack
        assert!(techniques
            .iter()
            .any(|t| matches!(t, AttackTechnique::QuickCrack { .. })));
    }
    ...
    fn test_recon_pipeline_is_passive() {
        ...
        assert!(!techniques
            .iter()
            .any(|t| matches!(t, AttackTechnique::EvilTwin { .. })));
