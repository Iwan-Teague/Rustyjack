...
const NEEDLES: [&str; 2] = ["unwrap(", "expect("];
const BASELINE_FILE: &str = "ci/unwrap_expect_baseline.txt";
const SKIP_DIRS: &[&str] = &[
    ".git",
    "ci",
    "target",
...
fn run() -> Result<(), String> {
    ...
    if count > baseline {
        return Err(format!(
            "unwrap/expect usage increased: baseline={} current={}",
            baseline, count
        ));
    }
    println!(
        "unwrap/expect usage OK: baseline={} current={}",
        baseline, count
    );
    Ok(())
}
...
fn count_in_file(path: &Path) -> Result<u64, String> {
    ...
    Ok(NEEDLES
        .iter()
        .map(|needle| count_occurrences(&content, needle))
        .sum())
}
