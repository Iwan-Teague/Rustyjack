...
impl App {
    ...
    fn run_ui_test_flow(&mut self, preset: TestPreset) -> Result<()> {
        ...
        let advanced = self
            .confirm_toggle(
                "Advanced opts",
                "Configure advanced per-suite options?",
                false,
            )?
            .unwrap_or(false);
        if advanced {
            self.collect_advanced_suite_args(&suites, &mut args, dangerous)?;
        }
    ...
    fn collect_advanced_suite_args(
        &mut self,
        suites: &[SuiteId],
        args: &mut Vec<String>,
        dangerous_enabled: bool,
    ) -> Result<()> {
        if suites.contains(&SuiteId::Wireless) {
            self.collect_skip_group(
                "Wireless",
                args,
                "--wireless-arg",
                &[
                    ("Skip wireless unit tests?", "--no-unit"),
                    ("Skip wireless integration tests?", "--no-integration"),
                    ("Skip wireless negative tests?", "--no-negative"),
                    ("Skip wireless isolation checks?", "--no-isolation"),
                    ("Skip wireless compatibility checks?", "--no-compat"),
                ],
            )?;
            let recon = self
                .confirm_toggle("Wireless", "Enable wireless recon tests?", false)?
                .unwrap_or(false);
            if recon {
                args.push("--wireless-arg".to_string());
                args.push("--recon".to_string());
            }
        }

        if suites.contains(&SuiteId::Ethernet) {
            self.collect_skip_group(
                "Ethernet",
                args,
                "--ethernet-arg",
                &[
                    ("Skip ethernet unit tests?", "--no-unit"),
                    ("Skip ethernet integration tests?", "--no-integration"),
                    ("Skip ethernet negative tests?", "--no-negative"),
                    ("Skip ethernet isolation checks?", "--no-isolation"),
                    ("Skip ethernet compatibility checks?", "--no-compat"),
                ],
            )?;
        }

        if suites.contains(&SuiteId::InterfaceSelect) {
            self.collect_skip_group(
                "Interface Select",
                args,
                "--iface-select-arg",
                &[
                    ("Skip interface-selection negative tests?", "--no-negative"),
                    (
                        "Skip interface-selection compatibility checks?",
                        "--no-compat",
                    ),
                    (
                        "Skip interface-selection isolation checks?",
                        "--no-isolation",
                    ),
                ],
            )?;
            if dangerous_enabled {
                args.push("--iface-select-arg".to_string());
                args.push("--allow-remote-switch".to_string());
            }
        }

        if suites.contains(&SuiteId::Encryption) {
            self.collect_skip_group(
                "Encryption",
                args,
                "--encryption-arg",
                &[
                    ("Skip encryption unit tests?", "--no-unit"),
                    ("Skip encryption integration tests?", "--no-integration"),
                    ("Skip encryption negative tests?", "--no-negative"),
                    ("Skip encryption isolation checks?", "--no-isolation"),
                    ("Skip encryption compatibility checks?", "--no-compat"),
                ],
            )?;
        }

        if suites.contains(&SuiteId::Loot) {
            self.collect_skip_group(
                "Loot",
                args,
                "--loot-arg",
                &[
                    ("Skip loot unit tests?", "--no-unit"),
                    ("Skip loot integration tests?", "--no-integration"),
                    ("Skip loot negative tests?", "--no-negative"),
                    ("Skip loot isolation checks?", "--no-isolation"),
                    ("Skip loot compatibility checks?", "--no-compat"),
                ],
            )?;
        }

        if suites.contains(&SuiteId::Mac) {
            self.collect_skip_group(
                "MAC Randomization",
                args,
                "--mac-arg",
                &[
                    ("Skip MAC unit tests?", "--no-unit"),
                    ("Skip MAC stress loop?", "--no-stress"),
                    ("Skip MAC negative tests?", "--no-negative"),
                    ("Skip MAC vendor tests?", "--no-vendor"),
                ],
            )?;
        }
        ...
        if suites.contains(&SuiteId::Daemon) {
            self.collect_skip_group(
                ...
                "--daemon-arg",
                &[
                    ("Skip daemon auth tests?", "--no-auth"),
                    ("Skip daemon protocol tests?", "--no-protocol"),
                    ("Skip daemon unit tests?", "--no-unit"),
                    ("Skip daemon compatibility checks?", "--no-compat"),
                    ("Skip daemon isolation checks?", "--no-isolation"),
                    (
                        "Skip daemon comprehensive sub-suite?",
                        "--skip-comprehensive",
                    ),
                ],
            )?;
        ...
        if suites.contains(&SuiteId::DaemonDeep) {
            let verbose = self
                .confirm_toggle("Daemon Deep", "Enable verbose deep daemon output?", false)?
                .unwrap_or(false);
            if verbose {
                args.push("--daemon-deep-arg".to_string());
                args.push("--verbose".to_string());
            }
        }

        if suites.contains(&SuiteId::Installers) {
            self.collect_skip_group(
                "Installers",
                args,
                "--installers-arg",
                &[
                    ("Skip shellcheck checks?", "--no-shellcheck"),
                    ("Skip syntax checks?", "--no-syntax"),
                    ("Skip pattern checks?", "--no-patterns"),
                    ("Skip installer isolation checks?", "--no-isolation"),
                ],
            )?;
        }

        if suites.contains(&SuiteId::Usb) {
            self.collect_skip_group(
                "USB",
                args,
                "--usb-arg",
                &[
                    ("Skip USB compatibility checks?", "--no-compat"),
                    ("Skip USB integration checks?", "--no-integration"),
                    ("Skip USB negative checks?", "--no-negative"),
                    ("Skip USB isolation checks?", "--no-isolation"),
                ],
            )?;
        }

        if suites.contains(&SuiteId::Theme) {
            self.collect_skip_group(
                "Theme",
                args,
                "--theme-arg",
                &[
                    ("Skip theme unit tests?", "--no-unit"),
                    ("Skip theme integration checks?", "--no-integration"),
                    ("Skip theme source checks?", "--no-source"),
                    ("Skip theme compatibility checks?", "--no-compat"),
                    ("Skip theme isolation checks?", "--no-isolation"),
                ],
            )?;
        }
    ...
    fn collect_skip_group(
        ...
        for (question, flag) in items {
            let skip = self
                .confirm_toggle(title, question, false)?
                .unwrap_or(false);
            if skip {
                args.push(suite_arg_key.to_string());
