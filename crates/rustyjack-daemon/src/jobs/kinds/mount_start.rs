    ...
    Fut: std::future::Future<Output = ()>,
{
    ...
    let mut handle = tokio::task::spawn_blocking(move || {
        rustyjack_core::services::mount::mount(
            request,
    ...
    let result = loop {
        tokio::select! {
            _ = cancel.cancelled(), if !cancel_notified => {
                ...
                progress("mount_start", 90, "Cancelling...").await;
            }
            res = &mut handle => {
                break res;
            }
    ...
    match result {
        ...
        Ok(Err(err)) => {
            Err(err.to_daemon_error_with_code(ErrorCode::MountFailed, "daemon.jobs.mount_start"))
        }
        Err(err) => Err(
