...
impl RouteManager {
    ...
    pub fn set_default_route(&self, iface: &str, gateway: Ipv4Addr, metric: u32) -> Result<()> {
        info!("Setting default route: via {} gw {} metric {}", iface, gateway, metric);
        
        let existing = self.ops.list_routes()
            .context("failed to list routes")?;
        
        for route in existing {
            if route.destination.is_none() {
                debug!("Removing existing default route via {}", route.interface);
                self.ops.delete_default_route(&route.interface)
                    .context("failed to delete existing default route")?;
            }
        }
        
        self.ops.add_default_route(iface, gateway, metric)
        ...
        match verification {
            ...
            Some(route) => {
                bail!("Route verification failed: expected {}, got {}", iface, route.interface);
            }
            None => {
    ...
    pub fn get_default_route(&self) -> Result<Option<RouteEntry>> {
        let routes = self.ops.list_routes()
            .context("failed to list routes")?;
