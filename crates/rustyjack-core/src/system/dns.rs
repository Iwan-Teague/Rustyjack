...
impl DnsManager {
    ...
    pub fn set_dns(&self, servers: &[Ipv4Addr]) -> Result<()> {
        ...
        
        {
            ...
            file.sync_all()
                .context("failed to sync temp resolv.conf")?;
        }
        
        fs::rename(&temp_path, &self.resolv_path)
            .context("failed to rename resolv.conf")?;
    ...
    pub fn verify_dns(&self) -> Result<Vec<Ipv4Addr>> {
        let content = fs::read_to_string(&self.resolv_path)
